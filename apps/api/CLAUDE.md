...
└── meeting-file/        # CQRS; вложенный ресурс /meetings/:meetingId/files, контроллер под @UseGuards(JwtAuthGuard)
    ...
    │   ├── whisper-command.ts    # чистая buildWhisperArgs — аргументы whisper-cli (-m модель, -l язык/auto, -nt, -f вход)
    │   ├── whisper-stt.service.ts # WhisperSttService implements SttService — whisper.cpp как подпроцесс (Фаза 1: happy path для WAV)
