...
- **Фоновая обработка записи (`meeting-file/processing`)**. Оба входа в очередь идут через одно событие `MeetingFileProcessingRequestedEvent { fileId }`: `CreateMeetingFileHandler` публикует его для только что загруженной `recording` (решение «нужна ли обработка» — здесь, у издателя), `ReprocessMeetingFileHandler` — после успешного сброса статуса. `MeetingFileProcessingRequestedHandler` безусловно кладёт `fileId` в `MeetingFileProcessingQueue` — in-process воркер без внешнего брокера (`concurrency = 1`), ведёт `pending → processing → done|failed` и по успеху пишет `transcriptText` в ту же строку. `SttService` (токен `STT_SERVICE`) — две реализации, выбор через `useFactory` в `MeetingFileModule` по `STT_ENGINE` (`stub` | `whisper`, дефолт `DEFAULT_STT_ENGINE` в `stt.service.ts`): `StubSttService` — детерминированная заглушка (транскрипт из метаданных файла, без чтения содержимого и без ветвления по `NODE_ENV` — локальный pre-commit идёт с `development`); `WhisperSttService` — реальная транскрибация whisper.cpp (`whisper-cli`) как внешний подпроцесс через порт `PROCESS_RUNNER` (`SpawnProcessRunner` поверх `child_process.spawn`, без Python), пути и язык — `WHISPER_BIN_PATH` / `WHISPER_MODEL_PATH` / `WHISPER_LANGUAGE` (пусто → авто-детект). Не-WAV вход (`needsConversion` по mime) сначала конвертируется в 16 кГц моно 16-bit WAV через системный `ffmpeg` (из `PATH`, без отдельной env) во временный каталог `mkdtemp` под `WHISPER_TMP_DIR` (пусто → `os.tmpdir()`); каталог задачи всегда удаляется в `finally` — «сирот»-WAV не остаётся ни после `done`, ни после `failed`. `SttInput` несёт `storageKey` (движок берёт файл через `FileStorageService.absolutePath`), `mimeType` и `signal?` (отмена). **Устойчивость (`WhisperSttService`)**: до создания временного каталога — `existsSync(WHISPER_BIN_PATH)` / `existsSync(WHISPER_MODEL_PATH)` (пусто или файла нет → `throw`); одна транскрибация ограничена `WHISPER_TIMEOUT_MS` (пусто/0/не число → `300000` мс в коде) через `AbortSignal.timeout`, объединённый с внешним `input.signal` (`AbortSignal.any`) и проброшенный в оба вызова `PROCESS_RUNNER`; `SpawnProcessRunner` по `abort` полагается на `SIGTERM` от `spawn` и добивает `SIGKILL` по grace-таймеру (`killGraceMs`, деф. 3 с). Любой сбой (нет бинарника/модели, `ffmpeg`/whisper ненулевой код, таймаут, пустой `stdout.trim()`) → `throw Error` без частичного результата; воркер переводит файл в `failed` (существующий `POST .../reprocess` перезапускает). `MeetingFileProcessingQueue` держит `AbortController` на текущую задачу и в `OnModuleDestroy` делает `abort()` (порядок: `stopped = true` → `abort()` → `await current`) — вместе с `app.enableShutdownHooks()` в `main.ts` это гасит подпроцессы `whisper`/`ffmpeg` при остановке приложения и по `SIGTERM`. Путь ошибки STT в e2e задаётся через `.overrideProvider(STT_SERVICE)` (двойник падает по маркеру `__stt_fail__` в имени) — в проде тестовых веток нет; e2e всегда идут на stub, whisper.cpp/ffmpeg в CI и pre-commit не нужны. `POST /meetings/:id/files/:fileId/reprocess` — атомарный `updateMany({ where: { status: failed }, data: { status: pending, transcriptText: null } })`; `count === 0` → `409 Conflict` (гонка/не тот статус не приводит к двойной постановке в очередь), иначе публикуется событие. `DELETE` уносит транскрипт вместе со строкой; если файл в этот момент в обработке — воркер ловит `P2025` и молча останавливается. `MeetingFileProcessingQueue` реализует `OnModuleDestroy` (флаг остановки + `abort()` текущей задачи + `await` её «догорания») — иначе e2e с `app.close()` в `afterEach` «догорают» и пишут в закрытый `PrismaClient`. Durability между рестартами не гарантируется: зависшие `pending`/`processing` не возобновляются.
