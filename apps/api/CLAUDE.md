...
│   │   │                    # UpdateUserProfileCommand — { userId, name }; UpdateUserAvatarCommand — { userId, avatarKey }
│   │   └── handlers/        # CreateUserHandler — prisma.user.create; UpdateUserProfileHandler — prisma.user.update (name);
│   │                        # UpdateUserAvatarHandler — prisma.user.update (avatarKey)
│   └── queries/
...
│   ├── profile.module.ts    # imports: [AuthModule, StorageModule, MulterModule.registerAsync] — limits.fileSize из MAX_UPLOAD_SIZE_BYTES (деф. 5 МиБ, →413), fileFilter по AVATAR_MIME_TO_EXT (→400); регистрирует Command/QueryHandlers
│   ├── profile.controller.ts # GET /users/me, PATCH /users/me, PUT /users/me/avatar (FileInterceptor('file')) — userId из request.user (JwtAuthGuard)
