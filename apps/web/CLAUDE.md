...
│   │   └── page.tsx
│   ├── profile/        # /profile — защищённая карточка профиля (аватар/инициалы, имя-или-email, email, дата регистрации, ссылка «Редактировать» → /profile/edit); рендерит <ProfileView />
│   │   ├── page.tsx
...
├── components/         # переиспользуемые React-компоненты (register-form.tsx, login-form.tsx, dashboard.tsx, meeting-details.tsx, meeting-files.tsx, profile-view.tsx, profile-edit.tsx — клиентские, на HeroUI; avatar.tsx — аватар пользователя: <img> по avatarSrc с фолбэком на инициалы)
...
Профиль (`/users/me`, все под `Authorization: Bearer`): `getMe(token)` → `Me` (`{ id, email, name, avatarUrl, createdAt }`) через `bearerRequest`; `avatarSrc(avatarUrl)` дописывает `API_URL` к относительному `avatarUrl` (картинку раздаёт сам сервис), `null` и абсолютные ссылки не трогает. Изменения: `updateProfileName(token, name)` → `PATCH /users/me` (`bearerJsonRequest` — POST/PATCH с JSON под Bearer), `changePassword(token, { currentPassword, newPassword })` → `POST /users/me/password` (`Promise<void>`), `uploadAvatar(token, file)` → `PUT /users/me/avatar` (multipart, поле `file`). Ошибки этих трёх пропускаются через `withFriendlyErrors` — подмена текста `ApiError` понятным сообщением по статусу (`400` — валидация имени/пароля или формат не image, `401` — сессия истекла / неверный текущий пароль, `413` — файл > 5 МБ); сетевую ошибку (`status === 0`) не трогает. `session.ts` профиль не хранит — он всегда тянется из API. Компонент `components/avatar.tsx` (`Avatar`) рисует картинку по `avatarSrc(avatarUrl)`, а при её отсутствии или ошибке загрузки (`onError`) — кружок с инициалами из `name` (иначе `email`); диаметр — проп `size` в px (по умолчанию 40), `alt` по умолчанию пустой (декоративный — рядом всегда есть подпись), при переданном `alt` контейнер получает `role="img"`.

Файлы встречи (`/meetings/:id/files`, все под `Authorization: Bearer`): `getMeetingFiles(meetingId, token)` → список `MeetingFile[]` (`bearerRequest`); каждый `MeetingFile` теперь включает поля `summaryStatus: MeetingFileStatus | null` и `summary: MeetingFileSummary | null` (тип `MeetingFileSummary { summary: string; decisions: string[]; actionItems: string[] }`); `uploadMeetingFile({ meetingId, file, type, accessToken, onProgress })` — `POST` multipart через **`XMLHttpRequest`** (нужен `upload.onprogress`); `type` (`recording` | `attachment`) определяет **компонент** (`detectFileType`: mime `audio/*`/`video/*`, при пустом mime — по расширению) с ручным переопределением, а не api-клиент; `413` / `400` / `0` мапятся в понятный текст в компоненте; `downloadMeetingFile(meetingId, fileId, token)` — эндпоинт под guard, поэтому качает `fetch` с заголовком и возвращает `{ blob, filename }` (имя из `Content-Disposition`), сам файл сохраняет вызывающий компонент через скрытый `<a download>`; `deleteMeetingFile` / `reprocessMeetingFile` / `resummarizeMeetingFile` — `DELETE` / `POST .../reprocess` / `POST .../resummarize` через `bearerRequest` с нужным методом (тело ошибки Nest разбирает `messagesFromBody`). `reprocess` для не-`failed` → `ApiError` со `status === 409`; `resummarize` для не-`done` или уже `processing` → 409.
