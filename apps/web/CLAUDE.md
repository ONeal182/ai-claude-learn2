...
- Конфиг фреймворка — `next.config.ts`; в нём `headers()` навешивает security-заголовки на все ответы (`Content-Security-Policy`, `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy`). Origin API в `img-src`/`connect-src` берётся из `NEXT_PUBLIC_API_URL`. CSP пока с `'unsafe-inline'`/`'unsafe-eval'` для script (рантайм Next); ужесточение через nonce — отдельной задачей.
